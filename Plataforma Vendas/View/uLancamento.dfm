object LancamentoPedido: TLancamentoPedido
  Left = 0
  Top = 0
  Caption = 'Lan'#231'amento de Pedido'
  ClientHeight = 621
  ClientWidth = 854
  Color = clBtnFace
  Constraints.MinHeight = 660
  Constraints.MinWidth = 870
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  WindowState = wsMaximized
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object ovP_Titulo: TPanel
    Left = 0
    Top = 0
    Width = 854
    Height = 73
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    Align = alTop
    BevelOuter = bvNone
    Color = clHighlight
    ParentBackground = False
    TabOrder = 0
    OnClick = ovP_TituloClick
    ExplicitLeft = -2
    ExplicitTop = 1
    object ovL_Titulo: TLabel
      Left = 23
      Top = 20
      Width = 252
      Height = 30
      Caption = 'Lan'#231'amento de Pedido'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = 30
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
  end
  object ovP_Produtos: TPanel
    Left = 0
    Top = 73
    Width = 854
    Height = 548
    Align = alClient
    Color = clWhite
    ParentBackground = False
    TabOrder = 1
    OnClick = ovP_ProdutosClick
    ExplicitTop = 76
    DesignSize = (
      854
      548)
    object ovL_Produtos: TLabel
      Left = 32
      Top = 31
      Width = 50
      Height = 16
      Caption = 'Produtos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = 16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object ovL_ValorTotal: TLabel
      Left = 327
      Top = 483
      Width = 58
      Height = 13
      Anchors = [akLeft, akBottom]
      Caption = 'Valor Total: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = 13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ExplicitTop = 485
    end
    object ovSB_Inserir: TSpeedButton
      Left = 722
      Top = 476
      Width = 113
      Height = 58
      Anchors = [akTop, akRight]
      Caption = 'Inserir'
      Flat = True
      Glyph.Data = {
        
        B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6}
      OnClick = ovSB_InserirClick
      ExplicitLeft = 728
    end
    object ovL_Despesas: TLabel
      Left = 181
      Top = 483
      Width = 82
      Height = 13
      Anchors = [akLeft, akBottom]
      Caption = 'Despesas Totais:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = 13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ExplicitTop = 485
    end
    object ovS_Carrinho: TSpeedButton
      Left = 672
      Top = 0
      Width = 163
      Height = 53
      Hint = 'Carrinho de compras'
      Anchors = [akTop, akRight]
      Caption = '0'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clHotLight
      Font.Height = 40
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6}
      ParentFont = False
      OnClick = ovS_CarrinhoClick
    end
    object ovL_Quantidade: TLabel
      Left = 32
      Top = 483
      Width = 60
      Height = 13
      Anchors = [akLeft, akBottom]
      Caption = 'Quantidade:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = 13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ExplicitTop = 485
    end
    object ListaProdutos: TListView
      Left = 23
      Top = 55
      Width = 812
      Height = 415
      Anchors = [akLeft, akTop, akRight, akBottom]
      Columns = <
        item
          Caption = 'Id'
        end
        item
          Caption = 'Nome'
          Width = 200
        end
        item
          AutoSize = True
          Caption = 'Descri'#231#227'o'
        end
        item
          Caption = 'Margem'
          Width = 100
        end
        item
          Alignment = taRightJustify
          Caption = 'Custos'
          Width = 150
        end>
      RowSelect = True
      TabOrder = 0
      ViewStyle = vsReport
      OnClick = ListaProdutosClick
      OnCustomDrawItem = ListaProdutosCustomDrawItem
      OnDblClick = ListaProdutosDblClick
    end
    object ovE_ValorTotal: TEdit
      Left = 320
      Top = 502
      Width = 121
      Height = 21
      Alignment = taRightJustify
      Anchors = [akLeft, akBottom]
      Enabled = False
      TabOrder = 1
      Text = 'R$ 0,00'
    end
    object ovE_Despesas: TEdit
      Left = 174
      Top = 502
      Width = 121
      Height = 21
      Alignment = taRightJustify
      Anchors = [akLeft, akBottom]
      TabOrder = 2
      Text = 'R$ 400,00'
      OnExit = ovE_DespesasExit
      OnKeyPress = ovE_DespesasKeyPress
    end
    object ovSplV_Carrinho: TSplitView
      Left = 853
      Top = 1
      Width = 0
      Height = 546
      Margins.Left = 0
      Margins.Top = 0
      Margins.Right = 0
      Margins.Bottom = 0
      BiDiMode = bdLeftToRight
      Color = clHighlight
      DoubleBuffered = False
      Locked = True
      Opened = False
      OpenedWidth = 550
      Padding.Left = 3
      ParentBiDiMode = False
      Placement = svpRight
      TabOrder = 3
      object ListaCarrinho: TListView
        Left = 3
        Top = 57
        Width = 547
        Height = 489
        Align = alClient
        BorderStyle = bsNone
        Columns = <
          item
            Caption = 'Id'
          end
          item
            AutoSize = True
            Caption = 'Nome'
          end
          item
            Caption = 'Quantidade'
            Width = 80
          end
          item
            Caption = 'Valor total'
            Width = 80
          end>
        RowSelect = True
        TabOrder = 0
        ViewStyle = vsReport
        OnCustomDrawItem = ListaCarrinhoCustomDrawItem
        ExplicitLeft = 0
      end
      object ovP_Compra: TPanel
        Left = 3
        Top = 0
        Width = 547
        Height = 57
        Margins.Left = 0
        Margins.Top = 0
        Margins.Right = 0
        Margins.Bottom = 0
        Align = alTop
        BevelOuter = bvNone
        BiDiMode = bdLeftToRight
        Color = clSkyBlue
        ParentBiDiMode = False
        ParentBackground = False
        TabOrder = 1
        ExplicitTop = -1
        object ovSB_Comprar: TSpeedButton
          Left = 200
          Top = -3
          Width = 121
          Height = 62
          Caption = 'Comprar'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = 16
          Font.Name = 'Tahoma'
          Font.Style = []
          Glyph.Data = {
            
            B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6}
          ParentFont = False
          OnClick = ovSB_ComprarClick
        end
        object ovSB_Excluir: TSpeedButton
          Left = 89
          Top = -4
          Width = 107
          Height = 63
          Caption = 'Excluir'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = 16
          Font.Name = 'Tahoma'
          Font.Style = []
          Glyph.Data = {
            
            B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6}
          ParentFont = False
          OnClick = ovSB_ExcluirClick
        end
        object ovSB_FechaSplit: TSpeedButton
          Left = -1
          Top = -3
          Width = 86
          Height = 61
          Flat = True
          Glyph.Data = {
            
            B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6B8B9B6}
          OnClick = ovSB_FechaSplitClick
        end
      end
    end
    object ovE_Quantidade: TEdit
      Left = 23
      Top = 502
      Width = 121
      Height = 21
      Alignment = taRightJustify
      Anchors = [akLeft, akBottom]
      TabOrder = 4
      Text = '1'
      OnKeyPress = ovE_QuantidadeKeyPress
    end
  end
end
